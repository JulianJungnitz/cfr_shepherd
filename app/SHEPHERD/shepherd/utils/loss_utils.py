...
def _construct_labels(candidate_embeddings, candidate_node_idx, correct_node_idx, mask):
    ...
    candidate_node_idx_flattened_unique, unique_ind = unique(
        candidate_node_idx_flattened, dim=0
    )
    candidate_embeddings_flattened_unique = candidate_embeddings_flattened[
        unique_ind, :
    ]

    # remove padding
    if candidate_node_idx_flattened_unique[0] == 0:
        candidate_embeddings_flattened_unique = candidate_embeddings_flattened_unique[
            1:, :
        ]
    ...
    label_idx = torch.where(
        candidate_node_idx_flattened_unique.unsqueeze(1)
        == correct_node_idx.unsqueeze(0),
        1,
        0,
    )
    ...
    return (
        candidate_node_idx_flattened_unique,
        candidate_embeddings_flattened_unique,
        label_idx,
    )
...
def _construct_disease_labels(disease_embedding, batch_disease_nid):
    ...
    label_idx = torch.where(
        batch_disease_nid_unique.T == batch_disease_nid_reshaped, 1, 0
    )
        ...
        for i in range(sims.shape[0]):
            ...
            pos_term = (
                ...
                * torch.log(
                    1
                    + torch.sum(
                        torch.exp(-self.pos_weight * (anchor_pos_sim - self.thresh))
                    )
                )
            )
            neg_term = (
                ...
                * torch.log(
                    1
                    + torch.sum(
                        torch.exp(self.neg_weight * (anchor_neg_sim - self.thresh))
                    )
                )
            )
...
def construct_batch_labels(
    candidate_embeddings, candidate_node_idx, correct_node_idx, mask
):
    """
    Format the batch to input into metric learning loss function
    """
    ...
    candidate_node_idx_flattened_unique = candidate_node_idx_flattened[
        candidate_node_idx_flattened.squeeze() != 0
    ]
    candidate_embeddings_flattened_unique = candidate_embeddings_flattened[
        candidate_node_idx_flattened.squeeze() != 0, :
    ]
    ...
    def compute_loss(
        ...
        if disease_embedding is None:  # phenotype-phenotypes
            ...
            if self.only_hard_distractors or use_candidate_list:
                ...
                candidate_disease_idx, candidate_disease_embeddings, labels = (
                    construct_batch_labels(
                        disease_embedding,
                        batch_cand_disease_nid,
                        batch_corr_disease_nid,
                        disease_mask,
        ...
        print(
            ...
            " Labels: ",
            labels,
            " Candidate disease idx: ",
            candidate_disease_idx,
            " Candidate disease embeddings: ",
            candidate_disease_embeddings,
        )
        return (
            loss_dict,
