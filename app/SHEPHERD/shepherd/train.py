...
def parse_args():
    ...
    parser.add_argument(
        "--edgelist", type=str, default=None, help="File with edge list"
    )
    parser.add_argument(
        "--node_map", type=str, default=None, help="File with node list"
    )
    parser.add_argument(
        "--saved_node_embeddings_path",
        type=str,
        default=None,
        help="Path within kg_embeddings folder to the saved KG embeddings",
    )
    ...
    parser.add_argument(
        "--aug_sim", type=str, default=None, help="File with the similarity dictionary"
    )
    parser.add_argument(
        "--aug_gene_by_deg", type=bool, default=False, help="Augment gene by degree"
    )
    parser.add_argument(
        "--aug_gene_w",
        type=float,
        default=0.7,
        help="Contribution of augmentation (gene)",
    )
    parser.add_argument(
        "--n_sim_genes", type=int, default=3, help="K similar genes for augmentation"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--n_transformer_heads", type=int, default=8, help="Number of transformer heads"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.3,
        help="Kappa (Only used for combined model with link prediction loss)",
    )
    ...
    parser.add_argument(
        ...
        default="phenotypes",
        type=str,
        choices=["phenotypes", "phenotypes_and_genes"],
    )
    parser.add_argument(
        "--graph_shema", default="primeKG", type=str, choices=["primeKG", "hauner"]
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Name of the best performing checkpoint",
    )
...
def load_patient_datasets(hparams, inference=False):
    ...
    if inference:
        ...
        train_dataset = PatientDataset(
            project_config.PROJECT_DIR / "patients" / hparams["train_data"],
            time=hparams["time"],
            patient_aggr_nodes=hparams["patient_aggr_nodes"],
        )
        val_dataset = PatientDataset(
            project_config.PROJECT_DIR / "patients" / hparams["validation_data"],
            time=hparams["time"],
            patient_aggr_nodes=hparams["patient_aggr_nodes"],
        )
    ...
    if inference:
        test_dataset = PatientDataset(
            project_config.PROJECT_DIR / "patients" / hparams["test_data"],
            time=hparams["time"],
            patient_aggr_nodes=hparams["patient_aggr_nodes"],
        )
    else:
...
def get_dataloaders(
    ...
    if hparams["spl"] is not None:
        ...
        spl = None
    if (
        hparams["spl_index"] is not None
        and (project_config.PROJECT_DIR / "patients" / hparams["spl_index"]).exists()
    ):
        with open(
            str(project_config.PROJECT_DIR / "patients" / hparams["spl_index"]), "rb"
        ) as input_file:
    ...
    if args.aug_sim is not None:
        with open(
            str(
                ...
                / "knowledge_graph/8.9.21_kg"
                / ("top_10_similar_genes_sim=%s.pkl" % args.aug_sim)
            ),
            "rb",
        ) as input_file:
    ...
    with open(
        project_config.KG_DIR / f"degree_dict_{project_config.CURR_KG}.pkl", "rb"
    ) as input_file:
    ...
    if inference:
        ...
        train_dataloader = PatientNeighborSampler(
            "train",
            all_data.edge_index[:, all_data.train_mask],
            all_data.edge_index[:, all_data.train_mask],
            sizes=hparams["neighbor_sampler_sizes"],
            patient_dataset=train_dataset,
            batch_size=batch_sz,
            sparse_sample=sparse_sample,
            do_filter_edges=hparams["filter_edges"],
            all_edge_attributes=all_data.edge_attr,
            n_nodes=n_nodes,
            relevant_node_idx=gene_phen_dis_node_idx,
            shuffle=shuffle,
            train_phenotype_counter=phenotype_counter,
            train_gene_counter=gene_counter,
            sample_edges_from_train_patients=hparams[
                "sample_edges_from_train_patients"
            ],
            num_workers=hparams["num_workers"],
            upsample_cand=hparams["upsample_cand"],
            n_cand_diseases=hparams["n_cand_diseases"],
            use_diseases=hparams["use_diseases"],
            nid_to_spl_dict=nid_to_spl_dict,
            gp_spl=spl,
            spl_indexing_dict=spl_indexing_dict,
            hparams=hparams,
        ...
        val_dataloader = PatientNeighborSampler(
            "val",
            all_data.edge_index,
            all_data.edge_index[:, all_data.val_mask],
            sizes=[-1, 10, 5],
            patient_dataset=val_dataset,
            batch_size=batch_sz,
            sparse_sample=sparse_sample,
            all_edge_attributes=all_data.edge_attr,
            n_nodes=n_nodes,
    ...
    if inference:
        sizes = [-1, 10, 5]
        print("SIZES: ", sizes)
        test_dataloader = PatientNeighborSampler(
            "test",
            all_data.edge_index,
            all_data.edge_index[:, all_data.test_mask],
            sizes=sizes,
            patient_dataset=test_dataset,
            ...
            shuffle=False,
            num_workers=hparams["num_workers"],
            n_cand_diseases=hparams["test_n_cand_diseases"],
            use_diseases=hparams["use_diseases"],
            nid_to_spl_dict=nid_to_spl_dict,
            gp_spl=spl,
            spl_indexing_dict=spl_indexing_dict,
...
def get_model(
    ...
    if hparams["model_type"] == "aligner":
        if load_from_checkpoint:
            comb_patient_model = CombinedGPAligner.load_from_checkpoint(
                checkpoint_path=str(Path(project_config.PROJECT_DIR / args.best_ckpt)),
                edge_attr_dict=edge_attr_dict,
                all_data=all_data,
                n_nodes=n_nodes,
                node_ckpt=hparams["saved_checkpoint_path"],
                node_hparams=node_hparams,
                hparams=hparams,
            )
        else:
            comb_patient_model = CombinedGPAligner(
                edge_attr_dict=edge_attr_dict,
                all_data=all_data,
                n_nodes=n_nodes,
                hparams=hparams,
        ...
        if load_from_checkpoint:
            comb_patient_model = CombinedPatientNCA.load_from_checkpoint(
                checkpoint_path=str(Path(project_config.PROJECT_DIR) / args.best_ckpt),
                all_data=all_data,
                edge_attr_dict=edge_attr_dict,
                n_nodes=n_nodes,
            ...
            comb_patient_model = CombinedPatientNCA(
                edge_attr_dict=edge_attr_dict,
                all_data=all_data,
                n_nodes=n_nodes,
                node_ckpt=hparams["saved_checkpoint_path"],
...
def train(args, hparams):
    ...
    gene_phen_dis_node_types = (
        ["Gene", "Phenotype", "Disease"]
        if hparams["graph_shema"] == "hauner"
        else ["gene/protein", "effect/phenotype", "disease"]
    )
    gene_phen_dis_node_idx = torch.LongTensor(
        nodes.loc[nodes["node_type"].isin(gene_phen_dis_node_types), "node_idx"].values
    )
    ...
    if args.resume != "":
        ...
        run_name = run_name + f"_seed_{args.seed}"
        run_name = (
            run_name.replace("5_candidates_mapped_only", "5cand_map")
            .replace("8.9.21_kgsolved_manual_baylor_nobgm_distractor_genes", "manual")
            .replace("patient_disease_NCA", "pd_NCA")
            .replace("_distractor", "")
        )
        wandb_logger = WandbLogger(
            name=run_name,
    ...
    nid_to_spl_dict = {
        ...
        for idx, nid in enumerate(
            nodes[nodes["node_type"] == gene_identifier]["node_idx"].tolist()
        )
    }
    train_dataset, val_dataset, test_dataset = load_patient_datasets(hparams)
    patient_train_dataloader, patient_val_dataloader, patient_test_dataloader = (
        get_dataloaders(
            hparams,
            all_data,
            nid_to_spl_dict,
    ...
    checkpoint_path = (
        project_config.PROJECT_DIR / "checkpoints" / hparams["model_type"] / run_name
    )
    hparams["checkpoint_path"] = checkpoint_path
    print("Checkpoint path: ", checkpoint_path)
    if not os.path.exists(
        project_config.PROJECT_DIR / "checkpoints" / hparams["model_type"]
    ):
        (project_config.PROJECT_DIR / "checkpoints" / hparams["model_type"]).mkdir()
    if not os.path.exists(checkpoint_path):
        checkpoint_path.mkdir()
    monitor_type = (
        "val/mrr"
        if args.run_type == "disease_characterization"
        or args.run_type == "patients_like_me"
        else "val/gp_val_epoch_mrr"
    )
    fname = (
        "epoch_{epoch:02d}-val_mrr_{val/mrr:.2f}"
        if args.run_type == "disease_characterization"
        or args.run_type == "patients_like_me"
        else "epoch={epoch:02d}-val_mrr={val/gp_val_epoch_mrr:.2f}"
    )
    ...
    #  Train
    patient_trainer.fit(
        comb_patient_model, patient_train_dataloader, patient_val_dataloader
    )
...
def inference(args, hparams):
    ...
    n_nodes = len(nodes["node_idx"].unique())
    identifiers = (
        ["gene/protein", "effect/phenotype", "disease"]
        if hparams["graph_shema"] == "primeKG"
        else ["Gene", "Phenotype", "Disease"]
    )
    gene_phen_dis_node_idx = torch.LongTensor(
        nodes.loc[nodes["node_type"].isin(identifiers), "node_idx"].values
    )
    ...
    # Get datasets
    train_dataset, val_dataset, test_dataset = load_patient_datasets(
    ...
    trainer = pl.Trainer(
        accelerator="auto", logger=wandb_logger, precision="bf16")
