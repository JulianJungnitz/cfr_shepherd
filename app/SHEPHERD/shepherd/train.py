    ...
    if hparams['model_type'] == 'aligner':
        if load_from_checkpoint: 
            comb_patient_model = CombinedGPAligner.load_from_checkpoint(checkpoint_path=str(Path(project_config.PROJECT_DIR /  args.best_ckpt)), 
                                    edge_attr_dict=edge_attr_dict, all_data=all_data, n_nodes=n_nodes, node_ckpt = hparams["saved_checkpoint_path"], node_hparams=node_hparams, hparams=hparams)
        else:
...
def inference(args, hparams):
    ...
    wandb_logger = WandbLogger(run_name, project=hparams["wandb_project_name"], entity=hparams["wandb_entity"], save_dir=hparams['wandb_save_dir'])
    print('Run name: ', run_name)
    hparams['run_name'] = run_name
