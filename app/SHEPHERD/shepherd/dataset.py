...
class PatientDataset(Dataset):
    ...
    def __init__(
        ...
        self.time = time
        self.patient_aggr_nodes = patient_aggr_nodes

        # create HPO to node_idx map
        with open(
            project_config.KG_DIR / f"hpo_to_idx_dict_{project_config.CURR_KG}.pkl",
            "rb",
        ) as handle:
        ...
        self.idx_to_hpo_dict = {
            v: self.hpo_to_name_dict[k] if k in self.hpo_to_name_dict else k
            for k, v in self.hpo_to_idx_dict.items()
        }
        ...
        with open(
            str(
                project_config.KG_DIR
                / f"ensembl_to_idx_dict_{project_config.CURR_KG}.pkl"
            ),
            "rb",
        ) as handle:
        ...
        self.idx_to_disease_dict = {
            v: self.disease_to_name_dict[k] if k in self.disease_to_name_dict else k
            for k, v in self.disease_to_idx_dict.items()
        }

        # degree dict from idx to degree - used for debugging
        # NOTE: may need to subtract 1 to index into this dict
        with open(
            str(project_config.KG_DIR / f"degree_dict_{project_config.CURR_KG}.pkl"),
            "rb",
        ) as handle:
        ...
        if all(
            ["true_genes" in patient for patient in self.patients]
        ):  # first check to make sure all patients have true genes
        ...
        if all(
            ["true_diseases" in patient for patient in self.patients]
        ):  # first check to make sure all patients have true diseases
            ...
            for patients in dis_to_patients.values():
                for p in patients:
                    self.patients_with_same_disease[p].extend(
                        [pat for pat in patients if pat != p]
                    )
    ...
    def __getitem__(self, idx):
        ...
        if "additional_labels" in patient:
            for label, values in patient["additional_labels"].items():
                if label == "n_hops_cand_g_p":
                    continue
                if values == None:
                    values = [[-1]]
                if type(values) != list:
                    values = [[values]]  #  wrap in list if needed
                if type(values) == list and (
                    len(values) == 0 or type(values[0]) != list
                ):
                    values = [values]
                additional_labels_dict[label] = values
            if "max_percent_phen_overlap_train" not in patient["additional_labels"]:
                additional_labels_dict["max_percent_phen_overlap_train"] = [[-1]]
        ...
        if "true_diseases" in patient:
            if self.needs_disease_mapping:
                orpha_diseases = [
                    ...
                    for d in patient["true_diseases"]
                ]
                mondo_diseases = [
                    mondo_d
                    for orpha_d in set(orpha_diseases).intersection(
                        set(self.orpha_mondo_map.keys())
                    )
                    for mondo_d in self.orpha_mondo_map[orpha_d]
                ]
            else:
        ...
        assert (
            len(node_idx_to_aggregate) >= 1
        ), f"There are no phenotypes (and genes) for patient: {patient}"
        ...
        if (
            len(correct_genes_node_idx) == 0
        ):  # no correct genes available for the patient
            label_idx = None
        else:
            if self.raw_data:
                label_idx = [
                    candidate_gene_node_idx.index(g) for g in correct_genes_node_idx
                ]
            else:
                label_idx = (
                    candidate_gene_node_idx == correct_genes_node_idx[0]
    ...
    def get_candidate_diseases(self, cand_type="all_kg_nodes"):
        if cand_type == "all_kg_nodes":
            all_kg_diseases_idx = np.unique(
                list(self.disease_to_idx_dict.values())
            )  # get idx of all diseases in KG
            ...
            orpha_mondo_idx = np.unique(
                [
                    self.disease_to_idx_dict[d]
                    for d in orpha_mondo_diseases
                    if d in self.disease_to_idx_dict
                ]
            )
