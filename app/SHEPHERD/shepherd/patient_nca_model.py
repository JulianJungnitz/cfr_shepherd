...
from utils.train_utils import (
    fit_umap,
    plot_softmax,
    mrr_vs_percent_overlap,
    plot_gene_rank_vs_x_intrain,
    plot_gene_rank_vs_hops,
    plot_degree_vs_attention,
    plot_nhops_to_gene_vs_attention,
    plot_gene_rank_vs_fraction_phenotype,
    plot_gene_rank_vs_numtrain,
    plot_gene_rank_vs_trainset,
)
...
class CombinedPatientNCA(pl.LightningModule):
    def __init__(
        self, edge_attr_dict, all_data, n_nodes=None, node_ckpt=None, hparams=None
    ):
        super().__init__()
        ...
        self.node_model = NodeEmbeder.load_from_checkpoint(
            checkpoint_path=node_ckpt,
            all_data=all_data,
        ...
        self.patient_model = PatientNCA(
            hparams,
            embed_dim=self.node_model.hparams.hp_dict["output"]
            * self.node_model.hparams.hp_dict["n_heads"],
        )
    ...
    def forward(self, batch):
        ...
        pad_outputs = torch.cat(
            [torch.zeros(1, outputs.size(1), device=outputs.device), outputs]
        )
        t1 = time.time()
        ...
        phenotype_embeddings = torch.index_select(
            pad_outputs, 0, batch.batch_pheno_nid.view(-1)
        ).view(batch_sz, max_n_phen, -1)
        if self.hparams.hparams["loss"] == "patient_disease_NCA":
            batch_sz, max_n_dx = batch.batch_cand_disease_nid.shape
            disease_embeddings = torch.index_select(
                pad_outputs, 0, batch.batch_cand_disease_nid.view(-1)
            ).view(batch_sz, max_n_dx, -1)
        else:
        ...
        (
            ...
            disease_mask,
            attn_weights,
        ) = self.patient_model.forward(
            phenotype_embeddings, disease_embeddings, phenotype_mask, disease_mask
        )
        t3 = time.time()

        if self.hparams.hparams["time"]:
            print(
                f"It takes {t1-t0:0.4f}s for the node model, {t2-t1:0.4f}s for indexing into the output, and {t3-t2:0.4f}s for the patient model forward."
            )
    ...
    def rank_diseases(self, disease_softmax, disease_mask, labels):
        disease_mask = (disease_mask.sum(dim=1) > 0).unsqueeze(
            -1
        )  # convert (batch, n_diseases) -> (batch, 1)
        disease_softmax = disease_softmax + (
            ~disease_mask * -100000
        )  # we want to rank the padded values last
        disease_ranks = torch.tensor(
            np.apply_along_axis(
                lambda row: rankdata(row * -1, method="average"),
                axis=1,
                arr=disease_softmax.detach().cpu().numpy(),
            )
        )
        if labels is None:
            ...
            correct_disease_ranks = [
                ranks[lab == 1] for ranks, lab in zip(disease_ranks, labels)
            ]
    ...
    def rank_patients(self, patient_softmax, labels):
        ...
        labels = labels * ~torch.eye(labels.shape[0], dtype=torch.bool).to(
            labels.device
        )  # don't consider label positive for patients with themselves
        patient_ranks = torch.tensor(
            np.apply_along_axis(
                lambda row: rankdata(row * -1, method="average"),
                axis=1,
                arr=patient_softmax.detach().cpu().numpy(),
            )
        )
        if labels is None:
            ...
            correct_patient_ranks = [
                ranks[lab == 1] for ranks, lab in zip(patient_ranks, labels)
            ]
    ...
    def _step(self, batch, step_type):
        ...
        use_candidate_list = self.hparams.hparams[
            "only_hard_distractors"
        ]  # True if step_type != 'train' else False
        if self.hparams.hparams["loss"] == "patient_disease_NCA":
            labels = batch.disease_one_hot_labels
        else:
            labels = batch.patient_labels
        loss, softmax, labels, candidate_disease_idx, candidate_disease_embeddings = (
            self.patient_model.calc_loss(
                batch,
                phenotype_embedding,
                disease_embeddings,
                disease_mask,
        ...
        if self.hparams.hparams["loss"] == "patient_disease_NCA":
            correct_ranks = self.rank_diseases(softmax, disease_mask, labels)
        else:
            correct_ranks, labels = self.rank_patients(softmax, labels)
        ...
        if step_type == "test":
            ...
            batch, raw_pred, pred = self.node_model.get_predictions(
            ...
            roc_score, ap_score, acc, f1 = calc_metrics(
                metric_pred.cpu().detach().numpy(), link_labels.cpu().detach().numpy()
            )
        ...
        if self.hparams.hparams["plot_gradients"]:
            for k, v in self.patient_model.state_dict().items():
                self.logger.experiment.log(
                    {
                        f"gradients/{step_type}.gradients.%s"
                        % k: wandb.Histogram(v.detach().cpu())
                    }
                )
        ...
        return (
            ...
            disease_mask,
            attn_weights,
            candidate_disease_idx,
            candidate_disease_embeddings,
        )
    ...
    def training_step(self, batch, batch_idx):
        (
            ...
            attn_weights,
            cand_disease_idx,
            cand_disease_embeddings,
        ) = self._step(batch, "train")
        print("Patient loss: ", patient_loss)
        # print("Node embedder loss: ", node_embedder_loss)
        loss = (self.hparams.hparams["lambda"] * node_embedder_loss) + (
            (1 - self.hparams.hparams["lambda"]) * patient_loss
        )
    ...
    def write_results_to_file(
        ...
        if save:
            run_folder = (
                Path(project_config.PROJECT_DIR)
                / "checkpoints"
                / "patient_NCA"
                / self.hparams.hparams["run_name"]
            )
        ...
        if self.hparams.hparams["loss"] == "patient_disease_NCA":
            cand_disease_names = [
                d for d_list in batch["cand_disease_names"] for d in d_list
            ]

            all_sims, all_diseases, all_patient_ids = [], [], []
            for patient_id, sims in zip(
                batch["patient_ids"], softmax
            ):  # batch['cand_disease_names'], disease_mask,
                sims = sims.tolist()
            ...
            for patient_id, sims in zip(batch["patient_ids"], softmax):
                patient_mask = torch.Tensor(
                    [p_id != patient_id for p_id in batch["patient_ids"]]
                ).bool()
                remaining_pats = [
                    p_id for p_id in batch["patient_ids"] if p_id != patient_id
                ]
            ...
            results_df = pd.DataFrame(
                {
                    "patient_id": all_patient_ids,
                    "candidate_patients": all_cand_pats,
                    "similarities": all_sims,
                }
            )
        ...
        if attn_weights is None:
            ...
            for patient_id, attn_w, phen_names, p_mask in zip(
                batch["patient_ids"],
                attn_weights,
                batch["phenotype_names"],
                phenotype_mask,
            ):
            ...
            if save:
                phen_df.to_csv(
                    Path(run_folder) / "phenotype_attention.csv", sep=",", index=False
                )
        ...
        for edge_attn in gat_attn:
            ...
            attn = attn.cpu()
            gat_attn_df = pd.DataFrame(
                {"source": edge_index[0, :], "target": edge_index[1, :]}
            )
            ...
            if save:
                gat_attn_df.to_csv(
                    Path(run_folder) / f"gat_attn_layer={layer}.csv",
                    sep=",",
                    index=False,
                )  # wandb.run.dir
            layer += 1
        ...
        if save:
            torch.save(
                batch["n_id"].cpu(), Path(run_folder) / "node_embeddings_idx.pth"
            )
            torch.save(node_embeddings.cpu(), Path(run_folder) / "node_embeddings.pth")
            torch.save(
                phenotype_embeddings.cpu(),
                Path(run_folder) / "phenotype_embeddings.pth",
            )
            if self.hparams.hparams["loss"] == "patient_disease_NCA":
                torch.save(
                    disease_embeddings.cpu(),
                    Path(run_folder) / "disease_embeddings.pth",
                )
        if self.hparams.hparams["loss"] == "patient_disease_NCA":
            disease_embeddings = disease_embeddings.cpu()
    ...
    def test_step(self, batch, batch_idx):
        ...
        batch_results = {
            "test/correct_ranks": correct_ranks,
    ...
    def predict_step(self, batch, batch_idx):
        ...
        results_df, phen_df, attn_dfs, phenotype_embeddings, disease_embeddings = (
    ...
    def _epoch_end(self, outputs, loop_type):
        correct_ranks = torch.cat(
            [ranks for x in outputs for ranks in x[f"{loop_type}/correct_ranks"]], dim=0
        )  # if len(ranks.shape) > 0 else ranks.unsqueeze(-1)
        correct_ranks_with_pad = [
            ranks if len(ranks.unsqueeze(-1)) > 0 else torch.tensor([-1])
            for x in outputs
            for ranks in x[f"{loop_type}/correct_ranks"]
        ]

        if loop_type == "test":

            batch_info = {
                "n_id": torch.cat([x[f"{loop_type}/n_id"] for x in outputs], dim=0),
                "patient_ids": [
                    pat for x in outputs for pat in x[f"{loop_type}/patient_ids"]
                ],
                "phenotype_names": [
                    pat
                    for x in outputs
                    for pat in x[f"{loop_type}/phenotype_names_degrees"]
                ],
                "cand_disease_names": (
                    [
                        pat
                        for x in outputs
                        for pat in x[f"{loop_type}/cand_disease_names"]
                    ]
                    if outputs[0][f"{loop_type}/cand_disease_names"] is not None
                    else None
                ),
            }
            ...
            phenotype_mask = [
                pat for x in outputs for pat in x[f"{loop_type}/phenotype_mask"]
            ]
            disease_mask = [
                pat for x in outputs for pat in x[f"{loop_type}/disease_mask"]
            ]
            attn_weights = [
                pat for x in outputs for pat in x[f"{loop_type}/attention_weights"]
            ]
            gat_attn = [pat for x in outputs for pat in x[f"{loop_type}/gat_attn"]]
            node_embeddings = torch.cat(
                [x[f"{loop_type}/node.embed"] for x in outputs], dim=0
            )
            phenotype_embedding = torch.cat(
                [x[f"{loop_type}/patient.phenotype_embed"] for x in outputs], dim=0
            )
            disease_embeddings = (
                torch.cat(
                    [x[f"{loop_type}/patient.disease_embed"] for x in outputs], dim=0
                )
                if outputs[0][f"{loop_type}/patient.disease_embed"] is not None
                else None
            )
            if self.hparams.hparams["loss"] == "patient_disease_NCA":
                cand_disease_batch_nid = torch.cat(
                    [x[f"{loop_type}/batch_cand_disease_nid"] for x in outputs], dim=0
                )
            else:
                cand_disease_batch_nid = None
            ...
            output_base = (
                "/home/ml499/public_repos/SHEPHERD/shepherd/results/patients_like_me"
            )
        ...
        if self.hparams.hparams["plot_patient_embed"]:
            ...
            correct_gene_names = [
                "None" if len(li) == 0 else "  |  ".join(li)
                for x in outputs
                for li in x[f"{loop_type}/corr_gene_names"]
            ]
            correct_disease_names = [
                "None" if len(li) == 0 else "  |  ".join(li)
                for x in outputs
                for li in x[f"{loop_type}/disease_names"]
            ]

            phenotype_names = [
                " | ".join([item[0] for item in li][0:6])
                for x in outputs
                for li in x[f"{loop_type}/phenotype_names_degrees"]
            ]  # only take first few for now because they don't all fit
            patient_label = {
            ...
            self.logger.experiment.log(
                {
                    f"{loop_type}/patient_embed": fit_umap(
                        phenotype_embedding, patient_label
                    )
                }
            )
        ...
        if self.hparams.hparams["plot_disease_embed"]:
            ...
            disease_embeddings = torch.cat(
                [x[f"{loop_type}/patient.disease_embed"] for x in outputs], dim=0
            )
            disease_batch_nid = torch.cat(
                [x[f"{loop_type}/batch_disease_nid"] for x in outputs], dim=0
            )
            cand_disease_batch_nid = torch.cat(
                [x[f"{loop_type}/batch_cand_disease_nid"] for x in outputs], dim=0
            )
            ...
            cand_disease_names = [
                item
                for x in outputs
                for li in x[f"{loop_type}/cand_disease_names"]
                for item in li
            ]
            correct_disease_names = [
                "None" if len(li) == 0 else "  |  ".join(li)
            ...
            patient_label = {
                "Node Type": ["Patient Phenotype"] * phenotype_embedding.shape[0]
                + ["Disease"] * disease_embeddings.shape[0],
                "Name": phenotype_names + cand_disease_names,
                "Correct Disease": correct_disease_names
                + ["NA"] * disease_embeddings.shape[0],
            }
            self.logger.experiment.log(
                {f"{loop_type}/patient_embed": fit_umap(patient_emb, patient_label)}
            )

        if (
            "plot_softmax" in self.hparams.hparams
            and self.hparams.hparams["plot_softmax"]
        ):
            softmax = [pat for x in outputs for pat in x[f"{loop_type}/softmax"]]
            softmax_diff = [s.max() - s.min() for s in softmax]
            softmax_top2_diff = [
                torch.topk(s, 2).values.max() - torch.topk(s, 2).values.min()
                for s in softmax
            ]
            softmax_top5_diff = [
                torch.topk(s, 5).values.max() - torch.topk(s, 5).values.min()
                for s in softmax
            ]
            self.logger.experiment.log(
                {f"{loop_type}/softmax_top2_diff": plot_softmax(softmax_top2_diff)}
            )
            self.logger.experiment.log(
                {f"{loop_type}/softmax_top5_diff": plot_softmax(softmax_top5_diff)}
            )
            self.logger.experiment.log(
                {f"{loop_type}/softmax_diff": plot_softmax(softmax_diff)}
            )

        if self.hparams.hparams["plot_attn_nhops"]:
            # plot phenotype attention vs n_hops to gene and degree
            attn_weights = [
                torch.split(x[f"{loop_type}/attention_weights"], 1) for x in outputs
            ]
            ...
            self.logger.experiment.log(
                {
                    f"{loop_type}_attn/attention weights": wandb.Histogram(
                        attn_weights_cpu_reshaped[attn_weights_cpu_reshaped != 0]
                    )
                }
            )
            self.logger.experiment.log(
                {
                    f"{loop_type}_attn/single patient attention weights": wandb.Histogram(
                        attn_weights[0]
                    )
                }
            )
