...
class CombinedPatientNCA(pl.LightningModule):
    ...
    def write_results_to_file(self, batch, softmax, correct_ranks, labels, phenotype_mask, disease_mask, attn_weights,  gat_attn, node_embeddings, phenotype_embeddings, disease_embeddings, save=True, loop_type='predict'):
        ...
        if save:
            run_folder = Path(project_config.PROJECT_DIR) / 'checkpoints' / 'patient_NCA' / self.hparams.hparams['run_name'] 
    ...
    def predict_step(self, batch, batch_idx):
        ...
        # [     0,      0,      0,  ...,  23731,  23732,  23733]]), e_id=tensor([36717836,    10946,    10961,  ..., 73435210, 73435336, 73435640]), edge_type=tensor([0, 0, 0,  ..., 4, 4, 4]), size=[199773, 23734])]
        ...
        correct_ranks, softmax, labels, node_embedder_loss, patient_loss, roc_score, ap_score, acc, f1, gat_attn, node_embeddings, phenotype_embedding, disease_embeddings, phenotype_mask, disease_mask, attn_weights, cand_disease_idx, cand_disease_embeddings  = self._step(batch, 'predict')
