...
def parse_args():
    ...
    parser.add_argument('--graph_shema', default="primeKG", type=str, help='Which graph schema to use')    
...
def predict(args):
    ...
    identifiers = ['gene/protein', 'effect/phenotype', 'disease'] if hparams['graph_shema'] == 'primeKG' else ['Gene', 'Phenotype', 'Disease']
    ...
    dataloader = PatientNeighborSampler('predict', all_data.edge_index, all_data.edge_index[:,all_data.test_mask], 
                                        sizes = [-1,10,5], dataset=dataset, batch_size = hparams['inference_batch_size'], sparse_sample = 0, 
    ...
    trainer = pl.Trainer(accelerator='auto', devices=hparams['n_gpus']  ) # accelerator="cpu"
